...
const TestimonialSectionkey = ({ title, subtitle, testimonials }) => {
  return (
                ...
                >
                  <div className="star">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <i
                        key={i}
                        className={i < item.rating ? "fas fa-star" : "fa-regular fa-star"}
                      ></i>
                    ))}
                  </div>
                  <p>{item.feedback}</p>
...
export default TestimonialSectionkey;
